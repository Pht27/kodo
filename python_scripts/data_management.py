...
def save_game(data):
    ...
    for i in range(4):
        ...
        teams = pd.concat(
            [
                teams,
                pd.DataFrame({"team_id": team_id, "party": party}, index=[team_id]),
            ],
            ignore_index=True,
        )
        team_round_relations = pd.concat(
            [
                team_round_relations,
                pd.DataFrame(
                    {
                        "team_round_id": team_round_relations_index,
                        "team_id": team_id,
                        "round_id": round_id,
                    },
                    index=[team_round_relations_index],
                ),
            ],
            ignore_index=True,
        )
        ...
        for player in data["teams"][i]["players"]:
            temp = all_players[all_players["name"] == player[: len(player) - 1]][
                "player_id"
            ]
            player_id = temp.loc[temp.index[0]]

            player_team_relations = pd.concat(
                [
                    player_team_relations,
                    pd.DataFrame(
                        {
                            "player_team_id": player_team_relations_index,
                            "team_id": team_id,
                            "player_id": player_id,
                        },
                        index=[player_team_relations_index],
                    ),
                ],
                ignore_index=True,
            )
...
def change_points_of_round(match_id, new_points):
    df = pd.read_csv(csv_file_rounds)
    df.loc[df["round_id"] == match_id, "points"] = new_points
    df.to_csv(csv_file_rounds, index=False)
