...
def add_player():
    ...
    if new_name:
        ...
        new_player = pd.DataFrame(
            [[new_id, new_name, True, 0]],
            columns=["player_id", "name", "active", "start_points"],
        )
...
def get_match_history_stats():
    ...
    data = get_match_history_infos(
        start_index=offset, end_index=offset + limit
...
def get_match_history_stats_for_player(player_id):
    ...
    data = get_match_history_infos(
        specific_player_id=player_id, start_index=offset, end_index=offset + limit
    ).to_dict(orient="split")
    return jsonify(data)
...
@app.route("/api/stats/weekdays/<int:player_id>", methods=["GET"])
def get_weekday_stats_for_specific_player(player_id):
    data = calc_weekday_stats_for_specific_player(player_id).to_dict(orient="records")
    return jsonify(data)


@app.route("/api/stats/daytime/<int:player_id>", methods=["GET"])
def get_daytime_stats_for_specific_player(player_id):
    data = calc_daytime_stats_for_specific_player(player_id).to_dict(orient="records")
    return jsonify(data)
